...
/*   Updated: 2024/07/07 17:58:13 by gbuczyns         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_client_state	g_client_state;

int	main(int ac, char *av[])
{
	pid_t	pid;

	g_client_state.ack_received = 0;
	if (ac != 3)
	{
	...
	ft_putendl_fd("Waiting for acknowledge...", STDOUT_FILENO);
	while (!g_client_state.ack_received)
	{
		usleep(10);
	}
}
...
{
	(void)sig;
	g_client_state.send_next = 1;
}

void	handle_sigusr2(int sig)
{
	(void)sig;
	g_client_state.ack_received = 1;
